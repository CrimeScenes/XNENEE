...
local function getPingAdjustedPrediction()
    local ping = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()
    return ping / 1000  -- Convert to seconds for time-to-impact calculations
end

local function calculateAcceleration(part)
    local previousPosition = part.Position
    local previousVelocity = part.Velocity
    task.wait(0.1)
    local newPosition = part.Position
    local newVelocity = part.Velocity
    local acceleration = (newVelocity - previousVelocity) / 0.1
    return acceleration
end

local function getPredictedPosition(part)
    local velocity = part.Velocity
    local acceleration = calculateAcceleration(part)
    local timeToPredict = getPingAdjustedPrediction()
    local smoothedVelocity = (velocity + part.Velocity) / 2
    return part.Position + (smoothedVelocity * timeToPredict) + (0.5 * acceleration * timeToPredict^2)
end
...
local function raycastToTarget(targetPosition)
    ...
    local direction = (targetPosition - origin).Unit * 5000
    local raycastResult = game.Workspace:Raycast(origin, direction)
    return raycastResult and raycastResult.Instance:IsDescendantOf(targetPosition.Parent)
end
                ...
                for _, partName in ipairs(AllBodyParts) do
                    ...
                    if part then
                        ...
                        if predictedPosition and isWithinBoxFOV(predictedPosition) and os.clock() - lastClickTime >= getgenv().Forbidden.TriggerBot.Settings.Cooldown then
                            if raycastToTarget(predictedPosition) then
